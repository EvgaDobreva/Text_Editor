...
size_t KEYWORDS_LEN=64;

UndoInfo new_undo_info (int x, int y, ActionType type, int index, string data) {
	UndoInfo undo_info;
	undo_info.x=x;
	undo_info.y=y;
	undo_info.type=type;
...
TextBuffer* new_debug_buffer (TextBuffer* debug_buffer, bool last_action_modified, vector< vector<string> > clipboard, vector<UndoInfo> undo_history) {
	if (debug_buffer) {
		debug_buffer->clear();
		...
		for(size_t i=0; i < undo_history.size(); i++) {
			...
			stringstream line;
			line << "  " << "x=" << undo_info.x << " " << "y=" << undo_info.y << " " << "type=" << undo_info.type << " "
				  << "index=" << undo_info.index << " " << "data=\"" << undo_info.data << '"';
			debug_buffer->insert_line(line.str());
		}
	}
	return debug_buffer;
...
void TextBuffer::load_file(string new_filename) {
...
void TextBuffer::update(int buffer_x, int width, vector< vector<string> > clipboard, TextBuffer* debug_buffer) {
	...
	new_debug_buffer(debug_buffer, &last_action_modified, clipboard, undo_history);
...
void TextBuffer::delete_line(vector< vector<string> >& clipboard) {
	UndoInfo undo_info=new_undo_info(x, y, ACTION_DELETE, clipboard.size(), "");
...
void TextBuffer::insert_char(char character) {
	if (last_action == ACTION_INSERT_CHAR) {
		if (character != ' ' && undo_history.back().data[undo_history.back().data.length() - 1] == ' ') {
			UndoInfo undo_info=new_undo_info(x, y, ACTION_INSERT_CHAR, 0, "");
...
void TextBuffer::key_backspace() {
	if (x == 0) {
		if (y > 0) {
			...
			UndoInfo undo_info=new_undo_info(x, y, ACTION_MERGE_LINE, 0, "");
			undo_history.push_back(undo_info);
		}
	}
	else {
		...
		((lines_[y][x] != ' ' && undo_history.back().data[0] != ' ') || (lines_[y][x] == ' ' && undo_history.back().data[0] == ' '))) {
			undo_history.back().data.insert(undo_history.back().data.begin(), lines_[y][x]);
			undo_history.back().x--;
		}
		else {
			UndoInfo undo_info=new_undo_info(x, y, ACTION_BACKSPACE, 0, "");
...
void TextBuffer::key_delete() {
	if (x < (int) lines_[y].size()) {
		if (last_action == ACTION_DELETE) {
			...
			UndoInfo undo_info=new_undo_info(x, y, ACTION_DELETE, 0, "");
			undo_info.data=lines_[y][x];
			undo_history.push_back(undo_info);
		}
...
void TextBuffer::key_enter() {
	UndoInfo undo_info=new_undo_info(x, y, ACTION_SPLIT_LINE, 0, "");
	undo_history.push_back(undo_info);
...
void TextBuffer::cut_selection(vector< vector<string> >& clipboard) {
	...
	UndoInfo undo_info=new_undo_info(x, y, ACTION_CUT, clipboard.size(), "");
	undo_history.push_back(undo_info);
	clipboard.push_back(copied);
...
void TextBuffer::paste_selection(vector< vector<string> >& clipboard, int index) {
	...
	UndoInfo undo_info=new_undo_info(x, y, ACTION_PASTE, index, "");
	undo_history.push_back(undo_info);
	...
	switch(undo_info.type) {
		...
		case ACTION_PASTE: {
			...
			if (pasted.size() > 1) {
				...
				lines_.erase(lines_.begin() + undo_info.y + 1, lines_.begin() + undo_info.y + pasted.size());
			}
			x=undo_info.x;
			y=undo_info.y;
		}
		...
		case ACTION_MERGE_LINE:
			...
			break;
		case ACTION_REPLACE:
			lines_[undo_info.y].replace(undo_info.x, undo_info.index, undo_info.data.substr(undo_info.index));
			x=undo_info.x;
			y=undo_info.y;
			break;
		case ACTION_UNDO:
...
void TextBuffer::redo(vector< vector<string> >& clipboard) {
	...
	switch(undo_info.type) {
		...
		case ACTION_DELETE:
			...
			y=undo_info.y;
			break;
		case ACTION_DELETE_LINE: break;
		case ACTION_MOVE: break;
		case ACTION_CUT: break;
		case ACTION_COPY:
		case ACTION_PASTE:
			x=undo_info.x;
			y=undo_info.y;
		...
		case ACTION_MERGE_LINE:
			...
			break;
		case ACTION_REPLACE:
			lines_[undo_info.y].replace(undo_info.x, undo_info.data.length()-undo_info.index, undo_info.data.substr(0, undo_info.index));
			x=undo_info.x;
			y=undo_info.y;
			break;
		case ACTION_UNDO:
...
bool TextBuffer::replace(string what, string with) {
	...
	if (pos != string::npos) {
		...
		UndoInfo undo_info=new_undo_info(pos, y, ACTION_REPLACE, with.length(), with+what);
		undo_history.push_back(undo_info);
	...
	for (size_t row=y+1; row < lines_.size(); row++) {
		...
		if (pos != string::npos) {
			UndoInfo undo_info=new_undo_info(pos, row, ACTION_REPLACE, with.length(), "");
			undo_info.data+=with;
			undo_info.data+=what;
			undo_history.push_back(undo_info);
