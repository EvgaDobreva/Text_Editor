...
void set_default_controls(map<string, KeyAction> *controls) {
	...
	(*controls)["^s"] = KEYACTION_WORD_FORWARD;
	(*controls)["Mw"] = KEYACTION_WORD_BACKWARDS;
	(*controls)["Ma"] = KEYACTION_LINE_BEGIN;
	(*controls)["Me"] = KEYACTION_LINE_END;
	(*controls)["^a"] = KEYACTION_FILE_BEGIN;
	(*controls)["^e"] = KEYACTION_FILE_END;
...
void execute_action (int character, TextBuffer &file_buffer, ostringstream &status, map<string, KeyAction> controls,
						bool &should_close_program, vector< vector<string> > &clipboard, string last_search, string last_replace_what) {
	string key;
	switch (character) {
	case '\e': {
		key = 'M';
	...
	case KEYACTION_REPLACE: {
		...
		string replace_with=input_line("Replace with");
			...
			if (replace_what != "") {
				...
				status << "Replaced " << replaced_count << " occurrences of \"" << replace_what
				 		 << "\" with \"" << replace_with	<< "\"";
			}
		}
	}
...
int main(int argc, char* argv[])
{
	...
	use_default_colors();
	start_color();
	init_pair(GREEN, COLOR_GREEN, -1); 		// data_types
	init_pair(RED, COLOR_RED, -1); 			// keywords
	init_pair(BLUE, COLOR_BLUE, -1); 		// numbers
	init_pair(YELLOW, COLOR_YELLOW, -1); 	// strings
	init_pair(MAGENTA, COLOR_MAGENTA, -1); // comments
	...
	while (!should_close_program) {
		...
		execute_action(character, file_buffer, status, controls, should_close_program, clipboard, last_search, last_replace_what);
